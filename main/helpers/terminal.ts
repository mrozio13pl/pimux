...
export async function createTerminal(
    ...
    const piStatusPort = input.mode === 'pi' ? await ensurePiStatusServer(ctx) : undefined;
...
export function writeTerminal(input: { terminalId: string; data: string }): void {
    terminals.get(input.terminalId)?.write(input.data);
}

export function resizeTerminal(input: { terminalId: string; cols: number; rows: number }): void {
    terminals.get(input.terminalId)?.resize(input.cols, input.rows);
}

export function killTerminal(input: { terminalId: string }): void {
